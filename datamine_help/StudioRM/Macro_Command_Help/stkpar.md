# STKPAR Macro Command

Retrieves substitution variables and their values from a system character format file.

The character file may be set up by the `!STKSAV` command from within a macro or menu, or it may be set up with a system editor. The variables from file totally replace any existing variables in the stack. 
...
Where `<filename>` is a system file name. It may be up to 56 characters long andy may contain pathnames. The `<filename>` may be substituted in whole or in part by substitution variables. 

The form of the system file is:
