...
To access this menu:
  ...
  * Right-click the **Points** folder in the **Sheets** or **Project Data** control bar (if your product has one, and is configured to use default menus).

Note: Double-clicking the Points folder displays the [Data Import](<../COMMON/data%20import%20dialog.md>) screen, allowing you to select a file for import using one of the data source drivers provided.
...
Add Vertices |  Add more points to the target overlay by digitizing them in 3D. This affects all overlays associated with the object data. Note: Click **Done** when you've finished.  
---|---  
Delete Last   
Vertex | Remove the most recently digitized point from the data object. This affects all overlays associated with the object data.  
Project to   
Wireframe or  
Section |  Project all points of the overlay's data object along the line of sight to intercept with a 3D section or wireframe surface, if possible. Note: Data that cannot be projected is left unchanged.  
Quick Legend or  
