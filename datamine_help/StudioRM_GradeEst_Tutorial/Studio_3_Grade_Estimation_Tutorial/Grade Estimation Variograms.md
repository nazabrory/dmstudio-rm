![](../HeaderCell.jpg) |  Grade Estimation - Variograms The variogram model parameter file  
---|---  
...
Spherical Model – Type 1 ![](../Images/GE_Variogram1.gif) |  The spherical model is defined by the range a, and the spatial variance C: γi(h) = C [1.5 x h / a - 0.5 x (h / a)3 ] if h≤a = C if h>a The five fields required are:
...
---|---  
Power Model – Type 2 ![](../Images/GE_Variogram2.gif) |  The power model is defined by a power a (0 < a < 2) and a positive slope C: γi(h) = C x ha The five fields required are:
...
Exponential Model – Type 3 ![](../Images/GE_Variogram3.gif) |  The exponential model is defined by parameter a and spatial variance C: γi(h) = C[1 - exp(-h/a)] The five fields required are:
...
Gaussian Model – Type 4 ![](../Images/GE_Variogram4.gif) |  The Gaussian model is defined by parameter a and spatial variance C: γi(h) = C [1 – exp(-h2 / a2)] The five fields required are:
...
De Wijsian Model – Type 5 ![](../Images/GE_Variogram5.gif) |  The De Wijsian Model is defined by parameter c: γi(h) = C x loge(h) h>1 = 0 h≤1 The four fields required are:
...
The first rotation is a conventional azimuth rotation of 20o around the Z-axis, the second rotation is a dip of 40 o around the new X-axis, and the final rotation is 60o around the new Y-axis. Ranges for structure 1 are 100m, 200m and 300m in the new X, Y and Z directions. The fields are:
...
The definition of the variogram model in ESTIMA does not allow anisotopic Co or C values for model types 1 to 4. An anisoptopic C value for the De Wijsian model (type 5) is acceptable because the model equation does not have any other anisotropic variables.
...
![](../Images/NextExercise.gif)[Proceed to the next section](<Grade%20Estimation%20Run%20Time%20Optimization.md>) (Run Time Optimization)
